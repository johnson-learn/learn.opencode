...
base = r'C:\Users\<用户名>\AppData\Local\Temp\opencode'
target = r'C:\Users\<用户名>\Desktop\NR-f40\系统消息-01-SSB-MIB-SIB1与OSI.html'
...
svg2 = open(os.path.join(base, 'fig2-new.svg'), encoding='utf-8').read()
...
svg8 = open(os.path.join(base, 'fig8-new.svg'), encoding='utf-8').read()
...
figdir = os.path.join(base, 'si-figs')
anchors = [
...
for a in anchors:
    ...
    seg = a[3] if len(a) > 3 else open(os.path.join(figdir, key + '.html'), encoding='utf-8').read()
    i = h.find(anchor)
    if i < 0:
        print('锚点缺失:', key)
        continue
