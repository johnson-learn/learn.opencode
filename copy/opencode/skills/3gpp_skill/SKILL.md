...
- 不处理 3GPP 之外的标准化组织（IEEE/ITU 等）——除非与 3GPP 直接相关。注：路由表中含 WiFi(802.11)/IEEE 检索等子技能模块，仅作为「与 3GPP 直接相关的无线/通信工程补充工具」引用，不替代本技能以 3GPP 为主的职责（详见路由表）
...
- **内容产出不限篇幅（AGENTS.md 铁律第10条，最高优先级）**：任何梳理/讲解/文档/HTML 均不限篇幅字数时间 token，无需用户反复强调。① 必须详尽展开、相似内容原样重列、禁止"与 XX 同构/详见上文"带过；② **多轮追加写厚**——单个回复写不完就在后续多个连续回复对同一文件反复 edit 追加直到真正详尽，禁止压缩到单薄即宣称完成；③ **禁止以搜索冒充理解**——协议/代码梳理必须基于全文通读与理解（读文件全文、理解执行流程/功能实现），关键字搜索仅作定位辅助，不得作为主要手段遗漏内容；④ 超大数据源按模块分批深读后分批整合为完整交付。
- **⭐ 公式强制 LaTeX 呈现铁律（最高优先级，用户多次点名，违反=交付不达标）**：3GPP 规范中大量字符与公式是**图片式（OLE/截图）**，纯文本提取会显示成伪公式。**任何含数学符号的内容（HTML/答案/表格/翻译/讲解），公式、符号、字母下标一律用 MathJax/LaTeX 呈现**（`$...$` 或 `\(...\)` 包裹），**禁止纯文本伪公式**，例如：`N_BWP^`/`N_ID^`、`n_RB`、`⌈⌋⌊⌉`、`log2(`、`×`、`μ`、`≤` 等裸写一律改写为标准 LaTeX（如 `$N_{BWP}^{size}$`、`$\lceil\log_2(n_{BWP})\rceil$`、`$n_{CRB}=n_{PRB}+N_{BWP}^{start}$`）。同一句内不得有的用 `$..$` 有的裸写。**交付前强制自检**：扫描正文残留伪公式（正则特征 `^`/`_`/`⌈⌊⌉⌋`/`log2(`/裸 `μ`/`≤≥×·` 等出现在正文文字而非 `$..$` 内），发现即改写成 `$..$` LaTeX 后再交付（识别端双轨提取见 files_skill references/dual-track-extraction.md，与 files_skill 的 MathJax 三件套必查并列的强制步骤）。
...
8. **主线串联结构（专题级）**：多讲次专题必须有一条贯穿主线，禁止各讲孤立堆料：
   - 主线 = 功能的生命周期问题链（如"为什么需要 → 怎么配 → 怎么算 → 怎么交付 → 全流程回顾"），每讲只回答主线一个问题
   - <b>主线必须落实到"网络侧-终端侧如何使用"（双视角落地）</b>：
     - 每个专题第 1 讲必须给"四问定位"表：含义（是什么）/ 作用（承载什么）/ 目的（为什么存在）/ 应用场景（eMBB/URLLC/mMTC/广播接入等 + 场景决定参数）
     - 机制讲次之后必须有"双视角落地讲次"：① 双端流程对照表（网络侧 N 步 ↔ 终端 N 步逐步对应）；② <b>参数双端使用总表</b>（每参数四列：配置方→配置给谁 | 网络侧如何使用 | 终端如何使用）；③ 网络内部决策（规范不规定但原理必须讲，标注【解读/推导】）；④ 核心网参与的参数（QoS/5QI/GBR → 调度权重）
...
11. **配置链 ASN.1 溯源呈现规范（NR-f40 多轮打磨的最终方案，通用）**：
   - ASN.1 原文**完整照贴、按链路先后顺序排列**，块与块之间放**居中加粗大箭头 ⇓**（`<div class="arr">⇓</div>`，34px 深蓝居中）
   - **同色成对标记（链走向一眼可读的核心）**：同一个类型名出现两处——上文**调用处**与下文**定义头**——用**同一底色加粗**标出（`<span class="p1/p2/...">`，浅黄/浅蓝/浅橙/浅绿/浅紫轮换），**一对一个颜色**；顺着同色对从起点读到终点就是配置链
   - **每个类型必须由上一级的字段或消息顶层引出，不得凭空出现**（RRCReconfiguration-IEs 等 -IEs 类型必须先贴其顶层消息 RRCReconfiguration ::= SEQUENCE 再由 criticalExtensions 引出——"凭空放 -IEs"被用户指出的教训）
   - 字段变量红（.rv）/类型蓝（.rt）/关键字紫（.asn）/注释绿（.rc）作为常规着色保留；**禁止"第 N 级"文字编号、禁止树形图、禁止多余颜色**（用户明确否定：层级隶属靠"顺序+色对+箭头"自然表达）
12. **HTML 输出要求**：全部统一见 files_skill「输出文件格式要求 → ### HTML 格式」条目（左侧目录栏 h2~h5 全层级、浅色、章节编号体系、正文排版、表格、MathJax 三件套、图片就近化、批量修改安全、讲次分层结构）——本 skill 不再重复登记，只执行
13. **原文核实强化（以本机标准文档库为准）**：
   - 关键事实用 **grep 穷尽检索**原文全文计数确认（如"CONTAINING CellGroupConfig"全文恰好 4 处=4 个载体），凭记忆写结论前必须核实
   - **版本差异与知识错误分治（用户裁定）**：版本差异（如 38.533 V16.4.0 与 f40 的时延数值）并列标注即可、不按外部意见改；**知识性错误必须按本机 f40 原文修正**（如 BWP indicator 码点：n_BWP=n_BWP,RRC+1 的 +1 是 initial 的码点，码点按 BWP-Id 升序含 Id0）
   - 外部勘误建议处理：只采纳"知识错误"类（经 f40 原文核实后修正），版本差异类不采纳
...
- 详见 `references/config-chain-template.md`（**协议配置链梳理通用模版**：源头信令分主线、完整链逐级树形分层、`字段:类型` 成对、用途区分。与铁律11分工：铁律11管"ASN.1 原文照贴+同色对溯源"（ASN.1 段内禁止树形图）；本模版管"配置链梳理呈现"（源头→容器→目标IE 链条可用树形分层）——二者对象不同，不冲突）
