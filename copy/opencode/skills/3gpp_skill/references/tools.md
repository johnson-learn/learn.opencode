...
| （可选）本地 3GPP 文档库 | 本地规范离线查阅（仅用户明确要求时；非默认依据） | 本机路径示例 `<3GPP文档库目录>\`（可配置） | `Test-Path` | 从 3GPP FTP 重新下载（流程见官网权威信息章节） |
| （可选）本地 6G 文档 | TR 22.870/38.914 存档 | 本机路径示例 `<项目目录>\temp\6G\`（可配置） | `Test-Path` | 按「FTP 访问技巧」重下 |
