...
8. **测试固件保护（✓ 双机实测踩坑）**：`tests/` 目录与 `tools/archive/` 的测试/归档源码含占位符 token（测试转换逻辑的固件，如 `test_update_skill.py` 里 `<项目目录>=E:\real` 期望值），双向转换会破坏其源码——path_convert 已内置 STATE_DIRS 跳过这两个目录，**勿删该跳过逻辑**；归档脚本中残留他机用户名（如 <用户名>）按 `<用户名>` 占位符修正
