...
- **片段四分类**（按特征识别）：
  1. **update_skill 标记片段**：片段以 `update_skill` 开头 → 执行双向更新流程（见下）；片段内同时含路径特征 → 先记录目标目录再双向更新
...
- **规则总结（示例）**：
  - `update_skill`（无冒号）= 仅双向更新
  - `update_skill：问题` = 先双向更新，再回答问题
  - `问题：update_skill` = 先回答问题，再双向更新
  - `update_skill：问题：update_skill` = 先双向更新（吸收远端）→ 回答问题（基于最新状态）→ 再双向更新（对齐收尾）；多标记依次类推
  - `update_skill：别反复修改：问题：update_skill` = 双向更新（零修改模式）→ 回答 → 双向更新（零修改模式）
