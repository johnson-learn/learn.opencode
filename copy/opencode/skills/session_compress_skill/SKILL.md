...
3. **收集当前会话业务上下文**：基于本会话已知的「已核实 + 已落地 + 用户确认」的业务结论、目录、输出文件、工具，按模板分类整理。**A 含【用户核心目标】**；**C 分三层**【任务状态】+【领域结论】+【业务经验/已踩坑】(可选，只记业务坑，不含框架坑)；**D 每个文件带【关键内容指向】**（指出该产物最重要的结论/看哪节）。
...
- **C 要点分三层**【任务状态】+【领域结论】+【业务经验/已踩坑】：只收"已核实(原文/实测) + 已落地 + 用户确认"的核心结论，禁止未验证推测；细节已落 D 文件产物的只写一句 "详见 D 的 <文件>" 指引，不重复全量复制；【业务经验/已踩坑】只记业务类的"问题→解决"，帮助新会话避免重蹈，**禁止记录框架 skill 的踩坑**；只含本任务业务结论，不含框架 skill 讨论。
- **D 文件**：只列本会话产生/修改的**业务输出文件**，不列框架 skill 的改动文件；每个文件条目下用**【关键内容指向】**说明该产物最重要的结论/结构在哪、新会话要看哪节。
- **E 工具分两块**：只列实际使用过的**业务工具名**（opencode 内置工具不写、框架运营工具不写、不写操作步骤），另加【复用资源】列本任务可复用脚本/中间产物路径+用途。
...
- C 是否仅"已核实 + 已落地 + 用户确认"？是否误写了未验证推测或"下一步"？【业务经验/已踩坑】是否只记业务坑、未混入框架坑？
- A 是否写了【用户核心目标】？D 每个文件是否带了【关键内容指向】？
