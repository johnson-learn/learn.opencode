...
| LibreOffice | 批量 doc/docx/pptx→PDF | `<LibreOffice目录>\program\soffice.com`（26.2.5） | `soffice.com --headless -env:UserInstallation=file:///C:\Temp/LO --version` | winget `TheDocumentFoundation.LibreOffice`（离线包放 <离线安装包目录> 同法） |
...
| Tesseract | OCRmyPDF 引擎 + 独立 OCR | `C:\Program Files\Tesseract-OCR\tesseract.exe`（5.4.0） | `& "C:\Program Files\Tesseract-OCR\tesseract.exe" --version` | `winget install UB-Mannheim.TesseractOCR` |
...
| WeasyPrint | HTML/CSS→矢量 PDF（无需浏览器，纯 Python 渲染） | python 包 69.0（依赖 MSYS2 GTK DLL） | `python -c "import weasyprint"`（需已设环境变量 `WEASYPRINT_DLL_DIRECTORIES=C:\msys64\ucrt64\bin`，本机 69.0 实测 PDF ✓） | `pip install weasyprint -i 清华源` + `winget install MSYS2.MSYS2` + MSYS2 内 `pacman -S mingw-w64-ucrt-x86_64-gtk3` + 永久设上述环境变量 |
