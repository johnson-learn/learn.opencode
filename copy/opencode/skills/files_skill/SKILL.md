...
- **样式完整性核对（重构/整合文件后必查，丢样式教训）**：整合或重构 HTML 后必须核对主 `<style>` 块完整性——以下类样式全套不得丢失：`pre/.orig/.jiexi/.zhuyi/.example/.q/.bridge/.mainline/.box/.calc/.ans/.src/.formula/.exam/.rv/.rt/.asn/.rc/.dot/.lv`（BWP/PDCCH 重构后主样式残缺、正文类无背景色的教训）；交付前逐文件检查 `pre`/`.orig`/`.lv` 存在
...
- **MathJax 三件套完整性（重大坑，用户点名写进 skill）**：HTML 交付/修改后必查三件套，缺任一则全文公式不渲染（浏览器显示原始 `$...$` 文本）：
  ...
  3. **分隔符一致性**：正文实际使用的公式分隔符（`\(...\)`/`$...$`/`$$...$$`）必须与配置块声明的分隔符一致——MathJax 3 默认只认 `\(\)`，正文用 `$` 而配置未声明 `$` 时全部不渲染（PDCCH 文件 480 个 `$` 公式不渲染的重大坑）
...
- **框架与主线（串全文要求，用户反复强调）**：多讲次专题必须"讲次为大章（h2）+ 讲内小节为子目录（h3 X.Y）"+ 主线三件套串全文——① 文首主线导览块（.mainline：问题链总览+讲次↔章节映射）；② 每讲开头"设问（承接第 X 讲）"；③ 每讲小结"把球交给下一讲"移交句、末讲"主线收口"；机械拼接的旧文件必须补三件套（PDCCH 平铺 65 节重构为 7 讲大章的教训）
