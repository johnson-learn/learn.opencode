...
- **左侧固定目录栏（sidebar）**：
  - 自动收集正文**全部 h2~h5**（一项不遗漏）生成树形目录：h2 一级加粗、h3/h4/h5 逐级缩进（padding-left 28/42/56px、字号/颜色渐浅）；每个标题加唯一锚点 id（按文档顺序 `sec-N` 编号）
...
- **正文排版（对齐参考排版，用户确认）**：
  - 正文**贴左开始**（`margin: 0 0 0 320px`，禁止 `margin: 0 auto` 居中——正文居中会与贴左的参考排版宽度明显不一致）
  ...
  - 侧栏占位用 `margin-left`（勿与正文 padding-left 叠加双重偏移）
