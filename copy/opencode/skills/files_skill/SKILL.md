...
- **含"输出"二字 → HTML 交付**：提问中出现"输出"二字时，最终答案必须以 HTML 文件输出（MathJax 渲染公式、规范排版，head 模板见「输出文件格式要求 → ### HTML 格式 → 公式标准显示」条目），内容详细、不限字数篇幅；HTML 保存到提问时所在工作目录并浏览器打开（用户另行指定目录时按用户指定）
- **内容产出不限篇幅（AGENTS.md 铁律第10条，最高优先级）**：任何识别/梳理/文档/HTML 产出均不限篇幅字数时间 token。① 必须详尽展开、相似内容原样重列、禁止简化带过；② **多轮追加写厚**——单个回复写不完就在后续多个连续回复对同一文件反复 edit 追加直到真正详尽，禁止压缩到单薄即宣称完成；③ **禁止以搜索冒充理解**——对文档/协议/代码的梳理必须基于全文通读与理解（read 读全文、理解结构与执行），关键点搜索仅作定位辅助不得遗漏内容；④ 超大数据集按模块分批深读后分批整合为完整交付。
- **⭐ 公式强制 LaTeX 呈现铁律（最高优先级，用户多次点名，违反=交付不达标）**：协议/标准文档（3GPP 等）中大量字符与公式是**图片式（OLE/截图）**，纯文本提取会显示成伪公式。**任何含数学符号的内容（HTML/答案/表格/翻译/讲解），公式、符号、字母下标一律用 MathJax/LaTeX 呈现**（`$...$` 或 `\(...\)` 包裹），**禁止纯文本伪公式**，例如：`N_BWP^`/`N_ID^`、`n_RB`、`⌈⌋⌊⌉`、`log2(`、`×`、`μ`、`≤` 等裸写一律改写为标准 LaTeX（如 `$N_{BWP}^{size}$`、`$\lceil\log_2(n_{BWP})\rceil$`、`$n_{CRB}=n_{PRB}+N_{BWP}^{start}$`）。同一句内不得有的用 `$..$` 有的裸写。**交付前强制自检**：扫描正文残留伪公式（正则特征 `^`/`_`/`⌈⌊⌉⌋`/`log2(`/裸 `μ`/`≤≥×·` 等出现在正文文字而非 `$..$` 内），发现即改写成 `$..$` LaTeX 后再交付（与 MathJax 三件套必查并列的强制步骤）。

## 输出文件格式要求（按输出格式分类）

> 本节按「输出文件的格式」分类登记要求条目：每种格式一个子节（### 格式名），前置条件写在该格式子节标题中。后续新增输出格式（docx/pptx/xlsx/md 等）时，在本章按同一结构追加对应格式子节，不得把格式要求散落到其他章节。

### HTML 格式（前置条件：输出文件为 HTML 格式时强制遵守）

- **左侧固定目录栏（sidebar）**：
  - 自动收集正文**全部 h2~h5**（一项不遗漏）生成树形目录：h2 一级加粗、h3/h4/h5 逐级缩进（padding-left 28/42/56px、字号/颜色渐浅）；每个标题加唯一锚点 id（按文档顺序 `sec-N` 编号）；**逐级缩进用行内 `style.padding-left`（如 h2=8/h3=30/h4=52px）实现，勿用文本空格前缀+嵌套 ul margin 混合（层级会乱）**
  - 点击目录项平滑滚动到对应章节并高亮；页面滚动时自动高亮当前章节（scrollspy）
  - 窗口 <1000px 时侧栏自动收起，左上角 ☰ 按钮展开
  - 正文内原静态目录块（div/nav 等 class="toc"）删除，避免与侧栏重复
  - 生成/修改后校验：目录项数 = h2~h5 总数、每个目录项锚点在正文实际存在（无死链）、点击定位与滚动高亮实测通过（Chrome headless CDP，大页面等待 ≥9 秒）
  - **目录可折叠（含子项的章节）**：有子章节的父级项配**折叠按钮**（收起 `▸` / 展开 `▾`，非 emoji、用三角字符），点击按钮仅收起/展开子目录、**父级标题本身仍是锚点链接**（点击仍跳转，二者分离互不干扰）；无子项的章节为普通单项。折叠用原生 JS（约 10~15 行，无外部依赖），默认展开、可单独收起；交付前实测折叠展开与链接跳转均正常（CDP）
- **目录颜色要求（浅色，不用深色）**：侧栏白/浅灰底（`#f7f9fc`）+ 深蓝字（`#123c63`/`#33506e`）；hover 浅灰蓝（`#e6edf5`）；当前章节高亮浅蓝底（`#cfe3f5`）+ 蓝左边框（`#2f7fb8`）——与正文浅色风格协调，禁止深色底
- **章节编号体系**：讲次为大章（h2）、讲内小节 h3（`X.Y`）、子小节 h4（`X.Y.Z`）、孙级 h5（`X.Y.Z.W`）——全局连续；由多个讲次/文件整合而成的 HTML 先合并再统一重排编号，禁止各讲次内部重新从 1 编号造成目录编号重复、禁止残留旧编号；重编号后正文中的"见 X.Y"节号引用同步更新
- **正文排版（对齐参考排版，用户确认）**：
  - 正文**贴左开始**（`margin: 0 0 0 320px`，禁止 `margin: 0 auto` 居中——正文居中会与贴左的参考排版宽度明显不一致）；**若正文用 grid 布局（`.layout{display:grid; grid-template-columns:300px 1fr}`）承载侧栏，则正文列由 grid 自动分配、不可再额外 `margin-left`（grid 列宽 + margin 双重占位会左空双倍，如 600px 空白，实测踩坑）**
  - `max-width: 1400px`（可再放宽）、`padding: 34px 46px 80px`、正文 `text-align: justify` 两端对齐
  - 全局 `*{box-sizing:border-box}`、`html{scroll-behavior:smooth}`
  - 侧栏占位用 `margin-left`（勿与正文 padding-left 叠加双重偏移）；**用 grid 承载侧栏时正文列勿再额外 `margin-left`（grid 已分配列宽，再 margin = 左空双倍）**
- **表格排版（统一宽度，用户点名）**：所有表格 `width:100%` 撑满内容区、字号 13.5px；斑马纹（偶数行浅底）+ hover 高亮（浅蓝 `#eef4f9`）+ 首列 nowrap；表头浅底加粗
- **协议原文/ASN.1 块统一背景色（用户确认）**：配置链等处的协议原文（ASN.1、规范条文）用**统一背景色**呈现，区别于普通代码块——`.chain pre{background:#fbf6e9;border-left:5px solid #c55a11;border-radius:0 6px 6px 0;padding:14px 18px;}`（浅米黄底+橙左边框）；正文内 .orig 引用块浅橙底（#fdf6ec）同系
- **配置链树形分层呈现（NR-f40 多轮打磨，通用）**：所有"配置链 / 协议层级链条"一律用上面 `.chain pre` 统一背景色块（或等价 pre 样式），且**块内必须是逐级分层树形**：
  - 一级一级独立成行，用 `└─` / `├─` 缩进表示层级；**禁止**把多个层级写成一行的 `a:b → c:d → e:f` 串联
  - **`字段名 : 类型名` 成对**（如 `spCellConfigCommon : ServingCellConfigCommon`）；字段名（小写）与类型名（大写）易混时，强调"以冒号右侧类型名为准"，注意 SIB 版 / 完整版后缀差异
  - 从根（源头消息）到叶（目标 IE）中间层**不跳过**；源头信令（SIB1 / RRCReconfiguration 等）写清
  - 每一级后可用括注说明作用（如 `(→Type1 CSS，RAR)`）
  - 配置链放在与正文同色系的 pre 块，区别于普通代码块（见上条背景色）
- **样式完整性核对（重构/整合文件后必查，丢样式教训）**：整合或重构 HTML 后必须核对主 `<style>` 块完整性——以下类样式全套不得丢失：`pre/.orig/.jiexi/.zhuyi/.example/.q/.bridge/.mainline/.box/.calc/.ans/.src/.formula/.exam/.rv/.rt/.asn/.rc/.dot/.lv`；交付前逐文件检查 `pre`/`.orig`/`.lv` 存在
- **布局变更后图表核对**：body 排版变更（贴左/加宽/去居中）后，必须核对全部图表与正文编排——内联 SVG 加 `svg{max-width:100%;height:auto;}` 自适应新宽度；图片 `img{max-width:96%}`；逐文件 CDP 实测无溢出、图完整显示
- **MathJax 三件套完整性（重大坑，用户点名写进 skill）**：HTML 交付/修改后必查三件套，缺任一则全文公式不渲染（浏览器显示原始 `$...$` 文本）：
  1. **配置块**：`window.MathJax = { tex: { inlineMath: [...], displayMath: [...] } }`（head 内）
  2. **加载脚本**：`<script src="...mathjax...">`（cdn.jsdelivr 或本地）
  3. **分隔符一致性**：正文实际使用的公式分隔符（`\(...\)`/`$...$`/`$$...$$`）必须与配置块声明的分隔符一致——MathJax 3 默认只认 `\(\)`，正文用 `$` 而配置未声明 `$` 时全部不渲染（曾经出现文档大量 `$` 公式全部不渲染的重大坑）
  - 检查方法：扫描每个 HTML——`window.MathJax` 是否存在、正文 `$` 计数与 `\(` 计数、配置声明的 inlineMath 是否覆盖正文实际使用的分隔符；修复：注入双分隔符配置（`inlineMath: [['$','$'],['\\(','\\)']]` + `displayMath: [['$$','$$']]`）最稳
- **辅助元素**：文末"返回顶部"圆形按钮（滚动 >400px 显示，点击平滑回顶）
- **图片就近化**：HTML 交付物的图片（SVG/PNG）存项目内 `figs\` 子目录并用**相对路径**引用——禁止引用系统临时目录（%TEMP% 会被系统清理，曾致全部图片丢失）
- **讲次分层结构**：多讲次专题"讲次为大章（h2）+ 讲内小节为子目录（h3 X.Y）"，平铺散节应先整合为大章分层，避免目录层级混乱
- **公式标准显示（head 模板）**：HTML head 必须含 MathJax 双分隔符配置 + 加载脚本（缺任一则全文公式不渲染，见上条 MathJax 三件套）：
  `<script>window.MathJax={tex:{inlineMath:[['$','$'],['\\(','\\)']],displayMath:[['$$','$$']]}};</script>` + `<script src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-chtml.js"></script>`；正文公式一律标准 LaTeX（禁止 `N_ID^`、`log2(`、`⌊⌋`、`×` 等纯文本伪公式——正文/答案/翻译段内的数学符号全部 `$...$` 或 `\(...\)` 包裹，同一句内不得有的包有的裸写）
- **批量 HTML 修改安全（连环事故教训）**：
  - 字符串定位（find/rfind）**必须断言存在且唯一**——anchor 丢失时 find 返回 -1 导致错位插入、文件尾污染（曾致整段正文被复制到 `</body>` 之后）
  - 每次替换后立即校验：标签配对（含 table 开闭）、`</body>` 唯一、`$` 数量偶数、重复块检测、浏览器 CDP 实测 DOM 结构
  - 改动前备份副本；CSS/JS 注入用 `replace(x, y, 1)` 且带幂等标记；遍历式脚本动多文件前先确认目标清单（用户要求"其它文件不许动"的教训）
  - 正则匹配标题标签用 `<hN(?:\s[^>]*)?>` 形式，且注意 JS 代码里的 `i<h2.length` 会被误匹配（先剔除 `<script>…</script>` 段或加负向前瞻）
  - PowerShell 脚本含中文必须 UTF-8 BOM（无 BOM 会被 PS5.1 按 GBK 读成乱码）
  - CDP 测试大页面（多公式 MathJax）时等待 ≥9 秒（3 秒不够导致误判 sidebar 丢失）
...
> **python-pptx 表格 + LibreOffice 渲染双坑（✓ 实测）**：
