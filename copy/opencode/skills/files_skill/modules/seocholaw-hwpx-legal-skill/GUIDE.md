...
한글(Hancom Office)의 **HWPX** 파일을 XML 직접 작성 중심으로 생성, 편집, 읽기하고,
**HWP**(바이너리) 파일을 읽기/텍스트 추출/HWPX 변환할 수 있는 스킬.

- **HWPX**: ZIP 기반 XML 컨테이너(OWPML 표준). 생성/편집/읽기 모두 지원.
- **HWP**: OLE2 바이너리 형식. 읽기/텍스트 추출 + HWPX 변환 지원. (생성/편집은 HWPX로 변환 후 작업)
