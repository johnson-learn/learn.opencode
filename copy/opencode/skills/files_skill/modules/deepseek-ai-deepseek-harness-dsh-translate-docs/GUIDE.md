...
The briefing-driven path matches guidance-corpus quality at a fraction of the cost; the [briefed-updates Agent Note](https://skillsmp.com/zh/creators/notes/implemented/process/2026-07-26-briefed-minimal-translation-updates.md) owns the benchmark evidence.
...
* **[docs/i18n/README.md](https://skillsmp.com/zh/docs/i18n/README.md)** — the pairing contract: the three-file pair (`foo.md`, `foo.zh.md`, `foo.i18n.yaml`), the consistency record's both-side blob hashes, the language-switcher lines, scope, and exclusions.
* **[docs/i18n/translation-rules.md](https://skillsmp.com/zh/docs/i18n/translation-rules.md)** — how to translate: faithfulness, structure preservation, terminology discipline, typography (MUST/SHOULD levels).
* **[docs/i18n/terminology.md](https://skillsmp.com/zh/docs/i18n/terminology.md)** — the terminology table, binding in both directions. Load it BEFORE translating, not when a term feels uncertain; the terms you don't notice are the ones that drift.
* **[docs/i18n/translation-prompt.md](https://skillsmp.com/zh/docs/i18n/translation-prompt.md)** — the automated pipeline's calibrated machine-consumed template. Agents using this skill do not render it; the terminology table is the only repository file the automated renderer injects, while this skill and `translation-rules.md` remain binding for agent-authored translations.
* **[dsh-prose-standard](https://skillsmp.com/zh/creators/deepseek-ai/dsh-prose-standard/SKILL.md)** — required prose coverage and editorial judgment. Apply it to both sides without adding or dropping source propositions.

#### Translate

* **Pass 1 — write, don't transpose.** Read a semantic unit, then restate it as a native technical author in the nearest [style sample's](https://skillsmp.com/zh/docs/i18n/style-samples.md) register. Preserve the required frame without forcing sentence-by-sentence correspondence.
...
* Every term in [terminology.md](https://skillsmp.com/zh/docs/i18n/terminology.md) renders exactly as specified. For a Chinese target, use the Chinese and first-occurrence columns; an unlisted term needs a citable Chinese OSS/vendor precedent or stays English under 「待定术语」. For an English target, use the English column and an established English technical term; preserve an ambiguous source term with a short gloss and list it as pending. Never invent a rendering inline.
...
3. No manifest entry is needed for an ordinary document: every in-scope source requires a pair. Change [scripts/translation-pairing.manifest.json](https://skillsmp.com/zh/scripts/translation-pairing.manifest.json) only when the owning policy documents a genuine generated, instructional, or bilingual-by-construction exclusion.
4. Before the PR: the touched pairs are green under the scoped check; `pnpm run doc-sync` (which includes the corpus-wide pairing check plus `verify-md-wrap`/`verify-md-links`) runs once at PR level per [dsh-pre-push-checks](https://skillsmp.com/zh/creators/deepseek-ai/dsh-pre-push-checks/SKILL.md), not inside each translation task.
...
Follow the [code-review reporting guidance](https://skillsmp.com/zh/creators/deepseek-ai/dsh-code-review/SKILL.md#reporting-findings): evaluate each comment on its merits, and for terminology comments, remember the terminology table is the contract — apply a reviewer's rendering decision to [terminology.md](https://skillsmp.com/zh/docs/i18n/terminology.md), not only to one file.
