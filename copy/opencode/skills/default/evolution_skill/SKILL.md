...
2. **五步固化（每步先输出结构化中间结果再动作；标记格式程序化强制）**：
   ...
   > `【第四步·流水】` → 追加 evolution_log.txt（**只增不改**：追加尾部，禁止替换既有条目——教训 2026-08-26：替换导致记录覆盖丢失）；**新条目采用结构化格式（2026-08-28 V4 方案甲'，经验健康引擎按字段精确扫描的数据源）**：
   > ```
   > [日期] 标题
   > - 状态：active | 待二次验证 | deprecated
   > - 场景数：N（判定四条件依据）
   > - 最后验证：日期 或 未验证
   > - 核心关键词：词1、词2（1~3 个，供程序化追踪）
   > - 归属：xxx_skill（分域健康监控分组依据，2026-09-01 起必填）
   > - 载体：改动文件清单
   > - 内容：经验要点
   > ```
...
> 本 skill 相关经验在 `<opencode配置目录>\skills\default\evolution_skill\evolution_log.txt` 中带「归属：evolution_skill」字段的条目；active 条目摘要：五步检查点程序化强制、判定四条件防过度进化、结构化条目六字段+归属、L1 领域自测精准触发。低活性/待验证条目由经验健康引擎（gate --check）按归属分组提示。
