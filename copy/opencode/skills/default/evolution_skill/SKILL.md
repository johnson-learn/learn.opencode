...
description: 智能进化协议执行技能（全局 skill，默认触发）。Use when 需要执行进化固化——经验归纳与六步固化（踩坑/更优路径/新工具/机制缺陷/违反协议）、工具登记（tools-manifest.md）、注册表更新（regedit.md）、配套文档同步、进化建议产出（合并/拆分/迁移）、或响应插件注入的进化检查任务。AGENTS.md 铁律第 2 条每次响应复盘进化发现需固化时自动调用本技能。
collaborates_with:
  - update_skill
---
...
2. **六步固化（每步先输出结构化中间结果再动作；标记格式程序化强制）**：
   ...
   > `【第三步·确认】` → **question 工具弹窗逐条让用户选择（用户定，规则文件写入前强制，禁止回答正文文字提问代替）**：每条拟固化经验一个弹窗问题，选项 **① 同意本条进化**（按模型方案写入）**② 跳过本条**（不写入任何规则文件，仅 evolution_log.txt 记录"用户跳过"）；用户可用弹窗自带输入框**填写/修改内容**（按用户填写内容写入）；多条经验时一次 question 调用带多个问题逐条弹；**用户未确认前不得 edit 任何进化相关文件**；输出 `{用户选择, 填写内容(如有)}`
...
3. **修改复盘核查（edit 完成后、自测前强制，用户定）**：改了什么/为什么改/有无误删误改无关内容？规则类内容是否已进全部应改载体（不只 evolution.md）？是否符合占位符/可移植性/归属二分铁律？配套文档是否同步？——核查通过才跑测试
