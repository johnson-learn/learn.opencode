...
2. **六步固化（每步先输出结构化中间结果再动作；标记格式程序化强制）**：
   > **固化检查点强制输出格式（2026-08-27 起由插件+evolution_gate --check-5step 程序化检测，缺步自动告警补做；2026-09-04 五步升级六步——新增第三步·确认弹窗）**：执行固化动作（声明"已固化"）时，响应中必须按序出现以下六个标记行，每行后跟该步的结构化中间结果；只做"无固化"声明（"进化：无新固化"）时不需要六步：
   > `【第一步·归纳】` → 输出 `{经验描述, 经验类型(规则|记录|知识), 触发场景}`，一句精确描述、通用化（无本机路径）；**先过固化判定四条件（2026-08-28 报告评审后新增；V2 报告后程序化强制）**：① ≥2 个不同场景出现（或 1 个场景但代价高/用户点名）② 可移植 ③ 不与已有规则重复 ④ 能写出触发条件与适用边界——任一不满足 → 仅追加 evolution_log.txt（事实类），不进规则文件、**不弹窗确认**；**必须显式输出结论行 `【判定四条件】场景数：X / 可移植：是 / 无重复：是 / 边界：明确`（gate --check-5step 程序化检测，缺声明告警）**
   ...
   > `【第三步·确认】` → **question 工具弹窗逐条让用户选择（用户 2026-09-04 定，规则文件写入前强制，禁止回答正文文字提问代替）**：每条拟固化经验一个弹窗问题，选项 **① 同意本条进化**（按模型方案写入）**② 跳过本条**（不写入任何规则文件，仅 evolution_log.txt 记录"用户跳过"）；用户可用弹窗自带输入框**填写/修改内容**（按用户填写内容写入）；多条经验时一次 question 调用带多个问题逐条弹；**用户未确认前不得 edit 任何进化相关文件**；输出 `{用户选择, 填写内容(如有)}`
   > `【第四步·edit】` → 按归属清单逐个 edit 目标文件（每个 edit 后立即自检：改了什么/是否误删无关内容）
   > `【第五步·流水】` → 追加 evolution_log.txt（**只增不改**：追加尾部，禁止替换既有条目——教训 2026-08-26：替换导致记录覆盖丢失）；**新条目采用结构化格式（2026-08-28 V4 方案甲'，经验健康引擎按字段精确扫描的数据源）**：
   > ```
   > [日期] 标题
   ...
   > - 内容：经验要点
   > ```
   > `【第六步·校验】` → 跑归属清单中的校验测试 + 行为实测（涉及命令实跑）；不通过立即修正；**含过期经验排查（2026-08-28 报告评审后新增）**：新经验与旧经验矛盾 → 以新验证结果为准修订旧条目 + 追加 deprecation 记录（只标记不删历史）；本会话推翻某旧经验 → 同样处理
