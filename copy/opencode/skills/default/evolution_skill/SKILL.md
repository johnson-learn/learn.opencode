...
description: 智能进化协议执行技能（全局 skill，默认触发）。Use when 需要执行进化固化——经验归纳与六步固化（14 类触发：踩坑/错误失败/新工具/新方法/新经验/新边界/更优路径/更新方法/机制缺陷/违反协议/规则未生效/用户指正/外部变化/机制自升级）、工具登记（tools-manifest.md）、注册表更新（regedit.md）、配套文档同步、进化建议产出（合并/拆分/迁移）、或响应插件注入的进化检查任务。AGENTS.md 铁律第 2 条每次响应复盘进化发现需固化时自动调用本技能。
collaborates_with:
  - update_skill
---
...
1. **触发确认**（14 类触发，兼具具体与广泛）：**失败问题类**：踩坑 / 错误失败 / 机制缺陷 / 规则未生效需加强；**新发现类**：新工具 / 新方法 / 新经验 / 新边界情况；**改进类**：更优路径 / 更新方法；**规则执行类**：违反协议；**用户反馈类**：用户点名指正；**外部变化类**：外部环境变化（网站/API/软件升级致原方法失效）；**机制自进化类**：进化机制自身升级——任一触发 → 需固化
2. **六步固化（每步先输出结构化中间结果再动作；标记格式程序化强制）**：
   > **固化检查点强制输出格式（由插件+evolution_gate --check-5step 程序化检测，缺步自动告警补做）**：执行固化动作（声明"已固化"）时，响应中必须按序出现以下六个标记行，每行后跟该步的结构化中间结果；只做"无固化"声明（"进化：无新固化"）时不需要六步：
   > `【第一步·归纳】` → 输出 `{经验描述, 经验类型(规则|记录|知识), 触发场景}`，一句精确描述、通用化（无本机路径）；**先过固化判定四条件（程序化强制）**：① ≥2 个不同场景出现（或 1 个场景但代价高/用户点名）② 可移植 ③ 不与已有规则重复 ④ 能写出触发条件与适用边界——任一不满足 → 仅追加 evolution_log.txt（事实类），不进规则文件；**追加流水前仍须弹窗确认**（2026-09-10 用户裁决：任何非用户明确要求的文件修改含流水追加均先弹窗）；**必须显式输出结论行 `【判定四条件】场景数：X / 可移植：是 / 无重复：是 / 边界：明确`（gate --check-5step 程序化检测，缺声明告警）**
   ...
   > `【第三步·确认】` → **question 工具弹窗逐条让用户选择（规则文件写入前强制，禁止回答正文文字提问代替）**：每条拟固化经验一个弹窗问题，选项 **① 同意本条进化**（按模型方案写入）**② 跳过本条**（不写入任何规则文件，仅 evolution_log.txt 记录"用户跳过"）；用户可用弹窗自带输入框**填写/修改内容**（按用户填写内容写入）；多条经验时一次 question 调用带多个问题逐条弹；**用户未确认前不得 edit 任何进化相关文件**；输出 `{用户选择, 填写内容(如有)}`
   > `【第四步·edit】` → 按归属清单逐个 edit 目标文件（每个 edit 后立即自检：改了什么/是否误删无关内容）
   > `【第五步·流水】` → 追加 evolution_log.txt（**只增不改**：追加尾部，禁止替换既有条目——教训：替换导致记录覆盖丢失）；**新条目采用结构化格式（经验健康引擎按字段精确扫描的数据源）**：
   > ```
   > [日期] 标题
   ...
   > - 归属：xxx_skill（分域健康监控分组依据，必填）
   > - 载体：改动文件清单
   > - 内容：经验要点
   > ```
   > `【第六步·校验】` → 跑归属清单中的校验测试 + 行为实测（涉及命令实跑）；不通过立即修正；**含过期经验排查**：新经验与旧经验矛盾 → 以新验证结果为准修订旧条目 + 追加 deprecation 记录（只标记不删历史）；本会话推翻某旧经验 → 同样处理
   - **evolution.md 规则文件更新附加铁律**：更新前必须 ① 结合 evolution_log.txt 核对 ② **弹窗让用户确认**（question 工具）③ 确认后才 edit ④ 更新后跑 test_evolution_consistency.py
3. **修改复盘核查（edit 完成后、自测前强制）**：改了什么/为什么改/有无误删误改无关内容？规则类内容是否已进全部应改载体（不只 evolution.md）？是否符合占位符/可移植性/归属二分铁律？配套文档是否同步？——核查通过才跑测试
...
- **注入量管控**：instructions/regedit/tools-manifest/docs-sync 四注入文件合计上限 70KB（2026-09-04 用户弹窗决策 50→70），超限触发精简（压缩示例/重复摘要，大表留速览头行）；health_check 检测告警
- **使用率自审**：进化检查时附带自查——本会话套用了哪些已固化经验；约 10 次会话未被提及的经验提示"低活性，建议下沉 references/ 或标 deprecated"
- **冲突显式裁决（弹窗决策）**：新经验与旧规则矛盾时禁止静默二选一——进化整理「冲突裁决建议」（冲突双方、新验证结果、修订方案）后 question 工具弹窗让用户决策（按新验证结果修订/保留旧规则/填写修改方案）；用户选"修订"才同步改旧条目，响应中显式标注"与旧规则 X 冲突，按新验证结果修订"
...
3. **纳入动作**（按四问结论）：
   ...
   - 新规则 md → regedit 登记生效方式（E 注入/F 按需）+ 70KB 注入量管控判定
...
| 能力 | 执行方式 |
|---|---|
| 自动更新（更优路径/新边界） | 弹窗确认后执行（六步流程第三步·确认） |
| 自动生成（全新领域经验 → 新 skill） | 弹窗确认后执行，按编写规范新建 |
| 自动合并（两 skill 高重叠） | 进化整理建议 → 弹窗（question 工具）让用户决策，选"执行"才动手 |
| 自动拆分（职责过多） | 进化整理建议 → 弹窗（question 工具）让用户决策，选"执行"才动手 |
| 跨层迁移（项目↔全局） | 进化整理建议 → 弹窗（question 工具）让用户决策，选"执行"才动手 |
...
- **经验写入进化相关文件前必须弹窗确认**（六步流程第三步·确认：question 工具逐条选择 同意/填写内容/跳过；未确认不得 edit；判定四条件不满足的纯流水追加也须弹窗确认——任何非用户明确要求的文件修改含流水追加均先弹窗）
- **所有需要用户决策的环节必须 question 工具弹窗**（合并/拆分/迁移/冲突裁决=进化整理建议后弹窗让用户决策，禁止只出文字建议等用户回复；选"执行"才动手）
- 进化检查任务（插件注入）不可跳过、不可精简
- 注册表自身也受本协议进化：分类可新增、条目按六步固化 + test_regedit.py 校验

## 本 skill 经验索引（分域健康监控台账）

> 本 skill 相关经验在 `<opencode配置目录>\skills\default\evolution_skill\evolution_log.txt` 中带「归属：evolution_skill」字段的条目；active 条目摘要：六步检查点程序化强制（含第三步·确认弹窗）、判定四条件防过度进化、结构化条目六字段+归属、L1 领域自测精准触发。低活性/待验证条目由经验健康引擎（gate --check）按归属分组提示。
