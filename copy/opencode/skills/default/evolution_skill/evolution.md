...
### 2026-09-04
1. **六步固化流程（五步升级六步）**：新增第三步·确认——任何经验写入进化相关文件（rules/skill/evolution.md/regedit.md 等）前必须用 question 工具弹窗让用户逐条选择：① 同意（按模型方案写入）② 填写内容（用户修改后写入）③ 跳过（仅流水记"用户跳过"）；用户未确认前不得 edit；判定四条件不满足的纯流水追加不弹窗。程序化配套：gate --check-5step（参数名保留历史名）检测六步标记，缺第三步·确认即告警；插件注入任务、SKILL.md/instructions/AGENTS/regedit 同步六步表述
2. **弹窗决策统一铁律**：凡需用户决策的环节一律 question 工具弹窗（禁止文字提醒/文字建议代替）——覆盖：① 合并/拆分/迁移=进化整理建议后弹窗让用户决策（执行/不执行/填写修改方案）② 规则冲突裁决=整理「冲突裁决建议」后弹窗（按新验证结果修订/保留旧规则/填写方案）③ 付费 API/凭证使用前弹窗征得同意 ④ update_skill 同步冲突/资产提取建议/网络失败询问等全部弹窗化
3. **门限触顶弹窗决策**：任何门限值（注入量上限/SKILL.md 体积门限/裸声明告警阈值等）一旦到门限值，必须 question 工具弹窗让用户选择——① 修改门限（用户填写新值，同步更新所有引用该门限的文件与测试）② 沿用原门限（按原门限继续精简/整改），禁止模型擅自改门限值或擅自突破；本次注入量门限 50→70KB（用户弹窗填写 70KB）已同步更新 instructions/regedit/tools-manifest/health_check.py/test_health_check.py
...
17. **进化质量与可持续性（2026-08-28 报告评审后新增）**：① 固化判定四条件（≥2 场景/可移植/不重复/有边界）——不满足只记流水事实类；② 经验状态标记（active/deprecated/invalidated，只追加标记不删改历史）；③ 重要经验二次验证（标待二次验证→下次任务验证→记结果）；④ 注入量管控（四注入文件合计上限 70KB——V2 修正 30→50KB，2026-09-04 用户弹窗决策改 50→70KB；触顶时弹窗让用户选改门限或沿用，见 2026-09-04 条目）；⑤ 使用率自审（约 10 次会话未被提及→提示下沉/标 deprecated）；⑥ 冲突显式裁决（禁止静默二选一，响应中标注并同步改旧条目；2026-09-04 起弹窗决策）
