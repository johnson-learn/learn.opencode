...
> **本文件定位（用户定）**：进化不是机械记录历史——本文件承载**最新进化的规则与机制**，
...
17. **进化质量与可持续性**：① 固化判定四条件（≥2 场景/可移植/不重复/有边界）——不满足只记流水事实类；② 经验状态标记（active/deprecated/invalidated，只追加标记不删改历史）；③ 重要经验二次验证（标待二次验证→下次任务验证→记结果）；④ 注入量管控（四注入文件合计上限 70KB；触顶时弹窗让用户选改门限或沿用）；⑤ 使用率自审（约 10 次会话未被提及→提示下沉/标 deprecated）；⑥ 冲突显式裁决（禁止静默二选一，响应中标注并同步改旧条目；弹窗决策）
