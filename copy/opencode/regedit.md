...
| test_setup_ps1.py | `tests\test_setup_ps1.py` | G | setup-windows.ps1 自动化测试（开关/部署范围/path_convert 体系/AST 语法/盘符动态探测/注册事件注入验证/必备工具缺失告警与退出码/w64Dir 独立变量/填写类检测排除 tests/工具类空值交互闭环/安装交互分级与 PowerShell 窗口安装/多源镜像直链第二渠道/失败回菜单/检测双通道/安装后自动配置，51/51） |
