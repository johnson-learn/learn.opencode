...
**任务窗强制（每个对话/每个要求都必须，随用户要求实时同步）**：① 每个对话与每个新要求都必须用 todowrite 建立任务窗跟踪进展（禁止无任务窗直接开启大任务）；② 任务量大时用子智能体（task 工具）分担并行，子智能体的进展也必须纳入同一任务窗跟踪；③ 用户更新要求时（新增/删改/变更需求）必须同步更新主智能体与子智能体的任务窗，使任务列表始终与最新要求及事实进展一致；④ 分块任务每块自闭环并立即标记对应项（in_progress/completed），完成一项必须立即同步更新——禁止创建后不管、禁止最后批量补标记。
