...
## 8. 修改复盘核查 + 测试先行 + 任务列表实时同步
每个文件修改完成后、跑自测之前，必须自我复盘核查该次修改（防低级错误）：
...
## 10. 内容产出不限篇幅/字数/时间/token（最高优先级铁律）
**任何内容产出任务（梳理/文档/HTML/代码分析/综述/讲解等）一律不做篇幅、字数、时间、token 的任何限制**，用户无需也不应被要求反复强调。执行约束：
- **必须详尽展开、严禁简化**：不得以"与 XX 同构/相同/详见上文"一句话带过；相似内容必须原样展开重列；事实、表格、流程、公式、差异点逐项详细论述。
- **多轮追加写厚**：当单个回复的文本量不足以写完目标文件时，必须**在后续多个连续回复中对同一目标文件反复追加内容**（edit 追加/分段 write），直到内容真正详尽、篇幅充足——禁止因"输出受限"把文件压缩到单薄状态即宣称完成。
- **不得用"内容搜索/关键词匹配"冒充"内容理解阅读"**：对代码/协议的梳理必须基于对实体内容的通读与理解（读文件全文、理解执行流程/功能实现），搜索仅作定位辅助，不得作为梳理的主要手段而遗漏大量内容。
- **数据/源码规模不受限于会话一次加载**：超大规模（如数十万行代码库）采用"按执行模块分批深读 + 分批整合"，每批真正读完并理解，最终合并为完整详尽交付。
- 本铁律适用于一切产出载体（HTML/Markdown/文本/表格），细则见 instructions.md「内容产出不限篇幅」章节与各输出技能（3gpp_skill / files_skill）。
