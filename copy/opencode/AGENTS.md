...
**任务窗强制（每个对话/每个要求都必须，随用户要求实时同步；动作化）**：
0. **每收到一条新要求/新消息，第一步就是调用 todowrite 为该要求建立/更新对应任务（含子智能体任务），标 in_progress**——先建/更新窗，再动工，禁止"先做后补窗"或"收到要求却不建窗"。
1. 每个对话与每个新要求都必须用 todowrite 建立任务窗跟踪进展（禁止无任务窗直接开启大任务）。
2. 任务量大时用子智能体（task 工具）分担并行，子智能体的进展也必须纳入同一任务窗跟踪。
3. 用户更新要求时（新增/删改/变更需求）必须同步更新主智能体与子智能体的任务窗，使任务列表始终与最新要求及事实进展一致。
4. 分块任务每块自闭环并立即标记对应项（in_progress/completed），完成一项必须立即同步更新——禁止创建后不管、禁止最后批量补标记。
