...
**执行分级铁律**：更新/生成=弹窗确认后执行（固化流程第三步·确认，用户定）；合并/拆分/迁移/冲突裁决=进化整理建议后 question 弹窗让用户决策，选"执行"才动手，禁止只出文字建议；任何进化动作不得附 git 同步。
...
1. **归纳**【第一步·归纳】：从本次会话提取可复用的新经验，用一两句话精确表述（按可移植性要求通用化）；**先过"固化判定四条件"（防过度进化/幸存者偏差）**：① 至少在 2 个不同场景出现过（或 1 个场景但踩坑代价高/用户点名）② 可移植（换台机器可照做）③ 不与已有规则重复 ④ 能明确写出触发条件与适用边界——**任一不满足 → 只追加 evolution_log.txt（事实类），不写规则文件，不弹窗确认**
2. **归属**【第二步·归属】：判断更新到哪个 skill 的哪个章节（工具依赖清单/处理流程/铁律/环境注意/路由表/访问技巧）或 instructions.md
3. **确认**【第三步·确认，用户定，规则文件写入前强制】：**question 工具弹窗**逐条让用户选择（禁止回答正文文字提问代替）——**① 同意本条进化**（按模型方案写入）**② 填写内容**（用户输入框填写/修改，按用户填写写入）**③ 跳过本条**（不写规则文件，仅流水记"用户跳过"）；多条经验一次调用多问题逐条弹；**未确认前不得 edit 任何进化相关文件（rules/skill/evolution.md/regedit.md 等）**；判定四条件不满足的事实类流水追加不需弹窗
...
- 外部付费 API 一律标注"需用户确认"——**使用前必须 question 工具弹窗征得用户同意**（用户定：凡需用户决策的环节一律弹窗，禁止文字提醒代替）
- 不写入任何密钥/token/密码/凭证
...
- **注入量管控**：系统提示注入四文件（instructions/regedit/tools-manifest/docs-sync）合计**上限 70KB**（用户定），超限时 health_check 告警；**门限触顶决策规则（用户定）**：任何门限值（注入量/SKILL.md 体积/告警阈值等）一旦到门限值，必须 **question 弹窗让用户选择**——「修改门限」（用户填写新值，同步更新所有引用该门限的文件与测试）或「沿用原门限」（按原门限继续精简/整改），禁止模型擅自改门限值或擅自突破
...
0.5 **字符边界规范细则（AGENTS.md 铁律第 9 条，用户定；细节以 AGENTS.md 为准）**：跨工具传数据一律文件化（禁内联中文/多行脚本；git commit 用 `-F`）；Python 写文件 `encoding="utf-8", newline="\n"`、读子进程输出 `encoding="utf-8", errors="replace"`；框架文本文件 UTF-8 无 BOM + LF（test_charset.py 防线，health_check 必跑）；临时文件放 `%LOCALAPPDATA%\Temp\opencode\`。
