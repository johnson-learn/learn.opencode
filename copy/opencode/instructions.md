...
> **规则三层与冲突优先级**：AGENTS.md 铁律 > regedit.md 注册表（加载方式权威）> 本文件（详版协议细则）。冲突时以高优先级层为准；本文件承载细则（六步固化流程/编写规范/回答规则），铁律摘要见 AGENTS.md，加载方式登记见 regedit.md。

## 智能进化协议（所有全局 skill 与指令文件自我进化，最高优先级约定）

> **注册表机制**：全体系组件的加载方式唯一权威登记在 `~\.config\opencode\regedit.md`（生效方式八分类 A~H：A 系统注入 100% 必达 / B 会话必读 / C 技能匹配 / D 显式调用 / E 运行时事件 / F 按需读取 / G 流程强制 / H 响应检查）。AGENTS.md 铁律第 0 条强制每次会话开始先读注册表；插件 session.created 程序化注入提醒兜底。组件新增/变更必须同步更新 regedit.md 并跑 `python <opencode配置目录>\tests\test_regedit.py`。
...
1. **自动更新**：任务中发现更优路径/新边界情况 → 按六步流程 edit 补充到对应 skill 章节（**写入前弹窗确认**）
2. **自动生成**：任务中发现全新经验/技能领域，符合全局 skill 要求（独立职责、可复用、非项目特定）→ 按编写规范新建全局 skill（**弹窗确认后执行**，放入 skills 目录后 skill-banner 自动纳入展示）
3. **自动合并（产出建议）**：两 skill 功能重叠度高且常同时被调用 → 进化整理「进化建议：合并 A 与 B 为通用 C」+ 理由方案 → **question 弹窗让用户决策**（执行/不执行/填写修改方案），选"执行"才动手
4. **自动拆分（产出建议）**：某 skill 职责过多、命中率下降 → 进化整理「进化建议：拆分 X 为 Y/Z」+ 职责划分 → **question 弹窗让用户决策**（执行/不执行/填写修改方案），选"执行"才动手
5. **跨层迁移（产出建议）**：项目 skill 通用经验 → 建议提升为全局；全局 skill 项目特定细节 → 建议下沉为项目 skill → 进化整理迁移建议 → **question 弹窗让用户决策**（执行/不执行/填写修改方案），选"执行"才动手

**执行分级铁律**：更新/生成=弹窗确认后执行（固化流程第三步·确认）；合并/拆分/迁移/冲突裁决=进化整理建议后 question 弹窗让用户决策，选"执行"才动手，禁止只出文字建议；任何进化动作不得附 git 同步。
...
- **经验表述必须通用化、可移植，不得局限本机**：
  ...
  - 本机绝对路径（`C:\Users\<用户名>\...`、`<盘符>:\...`）改用占位表述（如 `<用户目录>`、`<opencode配置目录>`）或标注"本机路径示例"
  - 区分两类内容：**通用经验**（任何机器直接适用）与**本机配置**（路径/版本/凭证，集中于工具依赖清单的本机位置列，移植时按清单重配）
  - 判断标准：换一台干净机器，经验能否照做？不能则重写为通用表述

### 归纳与固化流程（六步 + 每步校验自测，缺一不可）
> **六步检查点程序化强制**：执行固化（响应含"已固化"声明）时必须按序输出六个标记行，每行跟该步结构化中间结果；evolution_gate --check-5step（参数名保留历史名，检测内容为六步）由插件 session.idle 自动检测，缺步即注入补做警告。**第一步必须显式输出判定四条件结论行（`【判定四条件】场景数：X / 可移植：是 / 无重复：是 / 边界：明确`）——四条件是程序化检测项，缺声明同样告警**。只声明"无新固化/无固化项"时不需要六步。
1. **归纳**【第一步·归纳】：从本次会话提取可复用的新经验，用一两句话精确表述（按可移植性要求通用化）；**先过"固化判定四条件"（防过度进化/幸存者偏差）**：① 至少在 2 个不同场景出现过（或 1 个场景但踩坑代价高/用户点名）② 可移植（换台机器可照做）③ 不与已有规则重复 ④ 能明确写出触发条件与适用边界——**任一不满足 → 只追加 evolution_log.txt（事实类），不写规则文件；追加前同样必须弹窗确认（任何非用户明确要求的文件修改含流水追加均先弹窗，禁止先斩后奏）**
2. **归属**【第二步·归属】：判断更新到哪个 skill 的哪个章节（工具依赖清单/处理流程/铁律/环境注意/路由表/访问技巧）或 instructions.md
3. **确认**【第三步·确认，规则文件写入前强制】：**question 工具弹窗**逐条让用户选择（禁止回答正文文字提问代替）——**① 同意本条进化**（按模型方案写入）**② 填写内容**（用户输入框填写/修改，按用户填写写入）**③ 跳过本条**（不写规则文件，仅流水记"用户跳过"）；多条经验一次调用多问题逐条弹；**未确认前不得 edit 任何进化相关文件（rules/skill/evolution.md/regedit.md 等）**；**任何非用户明确要求的文件修改（含判定四条件不满足的事实类流水追加）均须先弹窗确认**
4. **更新**【第四步·edit】：用 edit 工具增补/修订对应章节，保持既有结构，不重写全文；新经验标注来源与验证状态
5. **记录**【第五步·流水】：追加到 `~\.config\opencode\skills\default\evolution_skill\evolution_log.txt`（日期 + 来源 + 更新点，只增不改）；规则/机制类经验同时提炼写入 `~\.config\opencode\skills\default\evolution_skill\evolution.md`（进化规则，更新前须弹窗确认）
6. **校验与自测（每条进化强制，不得跳过）**【第六步·校验】：
   - **内容正确性核查**：命令/路径/参数是否准确可执行；与既有条目有无矛盾重复；来源标注与验证状态标注是否齐全；通用性达标（无硬编码本机路径）；**过期经验排查**：本会话中新经验与旧经验矛盾时按风险规避节"以新验证结果为准同步修订旧条目"执行，并追加一条 deprecation 记录到 evolution_log（只标记不删历史）；本会话明显推翻了某旧经验时同样处理
...
- 外部付费 API 一律标注"需用户确认"——**使用前必须 question 工具弹窗征得用户同意**（凡需用户决策的环节一律弹窗，禁止文字提醒代替）
- 不写入任何密钥/token/密码/凭证
- 与既有内容矛盾时，以新验证结果为准并同步修订旧条目
- **重要经验二次验证**：写入 evolution.md 或铁律级规则的经验，标注"待二次验证"；下次遇到相关任务时主动套用验证，结果（生效/修正）追加 evolution_log；验证通过的标注改为"✓ 二次验证通过"

### 进化质量与可持续性机制
- **经验状态约定**：evolution_log 新条目默认 active；经验失效/被推翻时**只追加** deprecation/invalidated 记录并修订对应规则文件，**不删除不改写历史流水**（保持可追溯）；**被标记 deprecated 的经验必须在对应规则文件条目上显式加 `[DEPRECATED]` 前缀（或删除线）**（状态只写流水不可见，规则文件显式标记确保注入系统提示时 LLM 能看到）
- **注入量管控**：系统提示注入四文件（instructions/regedit/tools-manifest/docs-sync）合计**上限 70KB**，超限时 health_check 告警；**门限触顶决策规则**：任何门限值（注入量/SKILL.md 体积/告警阈值等）一旦到门限值，必须 **question 弹窗让用户选择**——「修改门限」（用户填写新值，同步更新所有引用该门限的文件与测试）或「沿用原门限」（按原门限继续精简/整改），禁止模型擅自改门限值或擅自突破
- **使用率追踪（程序化）**：插件 session.idle 用各条活跃经验的"核心关键词"对整会话 assistant 文本做关键词匹配，命中写 evolution_trace.jsonl；经验健康引擎按最后引用日期扫描低活性（>60 天未引用 → 提示下沉 references/ 或标注 deprecated）。LLM 只需在进化检查时响应引擎提示，不再依赖跨会话记忆自查
...
1. **语言跟随提问（回答语言硬约束）**：用户以何种语言提问，思考、回答、输出就必须以何种语言（中文提问→中文回答含思考过程，英文提问→英文回答），任何情况下不得因模型偏好/文档语言/任务习惯改用其它语言；协议原文、配置项名称、原始字段、ASN.1、代码、命令、报错信息等必要原文保持原样不翻译
   - **裁定条款（实测修正）**：语言跟随的唯一权威依据 = **当条消息的实际语言**，思考/回答/输出三者必须一致跟随；平台语言指令只是会话默认基调兜底，与当条消息实际语言不一致或平台检测失效时，一律以当条消息实际语言为准，不得沿用旧语言
...
4. **新项目 skill 注入规则（细节以 AGENTS.md 铁律第 6 条为准）**：新项目首次显式调用全局 skill → 执行全部全局 skill 注入（脚本 `python <opencode配置目录>\tools\inject_skills.py <目标项目目录>`，description 改默认触发），注入后提醒用户重启 opencode；已注入项目默认覆盖更新

5. **内容产出不限篇幅（AGENTS.md 铁律第 10 条详版，最高优先级）**：任何内容产出（梳理/文档/HTML/代码分析/综述/讲解）一律**不做篇幅、字数、时间、token 限制**，用户无需也不应被要求反复强调。强制执行：
   - **必须详尽展开、严禁简化**：相似内容必须原样展开重列、表格/流程/公式/差异点逐项详述，禁止"与 XX 同构/相同/详见上文"一句话带过。
   - **多轮追加写厚**：当单个回复文本量不足写完目标文件时，**必须在后续多个连续回复对同一目标文件反复 edit 追加**（不得因输出受限把文件压缩到单薄即宣称完成）；这是对抗模型单次输出上限的唯一正确手段。
   - **禁止以搜索冒充理解**：对代码/协议的梳理必须基于实体内容通读（读文件全文、理解执行流程与功能实现），关键字搜索仅作定位辅助，不得作为主要手段而遗漏大量内容。
   - **超大数据源分批深读**：超大代码库（数十万行）按执行模块分批 read 真正读完并理解，再分批整合为完整详尽交付，不得用一次加载不下作借口草草带过。
   - 本条目适用于一切产出载体与服务，所有输出类 skill（3gpp_skill/files_skill 等）必须纳入自身输出规则（见 skill 编写规范第 12 条）。
6. **长任务执行节奏（对抗"像中断"的观感，最高优先级）**：长任务中不得因"单次输出上限/上下文退化/反复确认"而显得频繁中断。强制执行：
   - **同一逻辑目标一轮内连续完成**：动手后一路做完该目标（读→理解→产出→校验→标记），不得中途停下"汇报/确认"交还控制权；把进展与汇报**攒到任务内里程碑**（真正完成后一次性汇报），不在每轮开头反复声明"继续/中断了"。
   - **批量工具调用减少往返**：能并行的工具（多个读、多个独立校验）一次消息内发出，不逐个往返。
   - **分块自闭环**：超大任务拆相对独立的分块，每块读→理解→产出→校验自己收尾并立即标记完成（实时同步 todo），避免"海量上下文+反复确认"叠加。
   - **任务窗强制（AGENTS.md 铁律第8条详版，每个对话/每个要求都必须；动作化）**：0. **每收到一条新要求/新消息，第一步就是调用 todowrite 为该要求建立/更新对应任务（含子智能体任务），标 in_progress**——先建/更新窗再动工，禁止"先做后补窗"；① 每个对话与每个新要求都必须用 todowrite 建立任务窗跟踪进展（禁止无任务窗直接开启大任务）；② 任务量大时用子智能体（task 工具）分担并行，子智能体进展也必须纳入任务窗跟踪；③ 用户更新要求时（新增/删改/变更需求）同步更新主智能体与子智能体任务窗，使任务列表始终与最新要求及事实进展一致；④ 分块自闭环并立即标记对应项，禁止创建后不管、禁止最后批量补标记。
   - **应对单次输出上限**：用铁律第10条"多轮对同一文件追加写厚"，不因输出受限把文件压薄即结束。
   - **应对超长上下文退化**：成果及时固化落文件，超大任务在合理节点开新会话从文件续做，不硬撑一个超长会话。
...
12. **内容产出不限篇幅**（所有输出类 skill 必须写入）：与「全局通用回答规则」第 5 条（AGENTS.md 铁律第 10 条）一致——输出 HTML/文档/梳理时不做篇幅字数限制，必须详尽展开、相似内容原样重列、多轮追加写厚、禁止以搜索冒充理解；3gpp_skill/files_skill 等输出技能必须把本条写入自身输出规则
...
- 全局目录（`~/.config/opencode/skills/` 等）中的 skill 为**全局 skill**；默认**不自动调用**，仅当用户用显式语法指定时才加载
- 显式语法：`<skill名>：<问题>`（中/英文冒号同）、`<skill名A>&<skill名B>：<问题>`（& 并列）、`<skill名A> <skill名B>：<问题>`（空格并列）
...
3. 技能名列表中未注册的名字：忽略该名字，其余照常加载；项目无 skill 且未显式指定：按普通问题正常处理
4. 新增全局专题 skill 时，仅需保证 SKILL.md 的 `name` 与目录一致，即自动支持本语法
