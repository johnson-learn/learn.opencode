...
- **经验状态约定**：evolution_log 新条目默认 active；经验失效/被推翻时**只追加** deprecation/invalidated 记录并修订对应规则文件，**不删除不改写历史流水**（保持可追溯）
- **注入量管控**：系统提示注入四文件（instructions/regedit/tools-manifest/docs-sync）合计**上限 30KB**，超出时触发精简——优先压缩示例段落、重复摘要与低引用条目，大表保留速览头行；health_check 检测注入总量并在超限时告警
