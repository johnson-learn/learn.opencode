...
3. **确认**【第三步·确认，规则文件写入前强制】：**question 工具弹窗**逐条让用户选择（禁止回答正文文字提问代替）——**① 同意本条进化**（按模型方案写入）**② 填写内容**（用户输入框填写/修改，按用户填写写入）**③ 跳过本条**（不写规则文件，仅流水记"用户跳过"）；多条经验一次调用多问题逐条弹；**未确认前不得 edit 任何进化相关文件（rules/skill/evolution.md/regedit.md 等）**；判定四条件不满足的事实类流水追加不需弹窗
