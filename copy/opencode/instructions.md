...
> **五步检查点程序化强制（2026-08-27 起；2026-08-28 V2 报告后加强）**：执行固化（响应含"已固化"声明）时必须按序输出五个标记行，每行跟该步结构化中间结果；evolution_gate --check-5step 由插件 session.idle 自动检测，缺步即注入补做警告。**第一步必须显式输出判定四条件结论行（`【判定四条件】场景数：X / 可移植：是 / 无重复：是 / 边界：明确`）——四条件是程序化检测项，缺声明同样告警**。只声明"无新固化/无固化项"时不需要五步。
1. **归纳**【第一步·归纳】：从本次会话提取可复用的新经验，用一两句话精确表述（按可移植性要求通用化）；**先过"固化判定四条件"（2026-08-28 报告评审后新增，防过度进化/幸存者偏差）**：① 至少在 2 个不同场景出现过（或 1 个场景但踩坑代价高/用户点名）② 可移植（换台机器可照做）③ 不与已有规则重复 ④ 能明确写出触发条件与适用边界——**任一不满足 → 只追加 evolution_log.txt（事实类），不写规则文件**
...
- **注入量管控**：系统提示注入四文件（instructions/regedit/tools-manifest/docs-sync）合计**上限 50KB**（2026-08-28 V2 报告修正：30KB 不现实——regedit+instructions 两个核心文件即 40KB；先控制总量不再增长，示例段与重复摘要按需精简），超出时触发精简；health_check 检测注入总量并在超限时告警
