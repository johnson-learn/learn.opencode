...
6. **长任务执行节奏（对抗"像中断"的观感，最高优先级）**：长任务中不得因"单次输出上限/上下文退化/反复确认"而显得频繁中断。强制执行：
   ...
   - **任务窗强制（AGENTS.md 铁律第8条详版，每个对话/每个要求都必须）**：① 每个对话与每个新要求都必须用 todowrite 建立任务窗跟踪进展（禁止无任务窗直接开启大任务）；② 任务量大时用子智能体（task 工具）分担并行，子智能体进展也必须纳入任务窗跟踪；③ 用户更新要求时（新增/删改/变更需求）同步更新主智能体与子智能体任务窗，使任务列表始终与最新要求及事实进展一致；④ 分块自闭环并立即标记对应项，禁止创建后不管、禁止最后批量补标记。
