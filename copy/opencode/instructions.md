...
- **主机制：全局 AGENTS.md 铁律注入**。`~\.config\opencode\AGENTS.md` 会被 opencode 作为全局规则加载进**每个会话的系统提示**（已验证：会话上下文可见其全文）。第 2 条即"每次响应后强制复盘进化"，执行点=**每次回答结束前自查**（踩坑/更优路径/新工具/机制缺陷/违反协议 → 立即五步固化并在回答末尾附"进化：已固化…/无新固化"）。这是唯一每次都必然对模型可见的机制。
