...
# 2. AGENTS.md 铁律 0~10 共 11 条（用整型排序，避免字符串字典序把 "10" 排在 "2" 前）
nums = re.findall(r"^## (\d+)\. ", agents, re.M)
check("AGENTS.md 铁律 0~10 共 11 条", sorted(map(int, nums)) == list(range(11)))

# 3. instructions.md 引用的铁律编号不超过 10 且指向正确条款
refs = re.findall(r"铁律第\s*(\d+)\s*条", ins)
check("instructions 引用铁律编号合法", all(int(n) <= 10 for n in refs))
# 第 2 条=复盘进化 的对照
