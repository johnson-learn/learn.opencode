...
| setup-windows.ps1（开关完整性/部署范围/path_convert 体系/AST 语法/盘符动态探测/注册事件注入验证/必备工具缺失告警/w64Dir 独立变量/填写类检测排除 tests/工具类空值交互闭环/安装交互分级与 PowerShell 窗口安装/多源镜像直链/全工具版本动态解析/管理员提权安装窗口/失败回菜单/检测双通道/安装后自动配置/模拟测试执行/窗口统一管理与出口清理） | `test_setup_ps1.py` | `python tests\test_setup_ps1.py` | ✓ 57/57 |
| setup 安装核心逻辑模拟测试（mock curl/Start-Process/Stop-Process 实际执行分支流转+窗口管理+worker 停止信号自毁） | `test_setup_sim.ps1` | `powershell -NoProfile -File tests\test_setup_sim.ps1 -FuncFile <仓库>\copy\setup\setup-install-functions.ps1` | ✓ 20/20 |
