...
| skill-banner 插件（事件处理/任务注入/日志落盘/注册事件系统提示注入/五步检查点/API 风险告警闭环/待办内存传递/平台语言检测持续约束/新增文件适配第 7 项/使用率追踪） | `test_plugin.js` | `node tests\test_plugin.js`（需 node） | ✓ 49/49（事件分支 20 + 注册事件注入 14 + 五步/API/待办 7 + 语言检测 5 + 使用率追踪 3） |
