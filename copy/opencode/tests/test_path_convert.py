...
HOME = os.environ.get("USERPROFILE", "<工具目录>\Users\\default").replace("/", "\\")
...
open(f2, "w", encoding="utf-8").write("配置 <opencode配置目录> 与临时 <用户临时目录>")
...
open(os.path.join(walk_dir, "path_map.txt"), "w", encoding="utf-8").write("<项目目录>=" + HOME + "\\x")
...
check("path_map.txt 未被动", ("<项目目录>=" + HOME + "\\x") in open(os.path.join(walk_dir, "path_map.txt"), encoding="utf-8").read())
...
open(os.path.join(scan_dir, "s.md"), "w", encoding="utf-8").write("残留 <未定义目录> 与 <用户目录>")
unk = pc.scan_unknown_placeholders(scan_dir)
check("检出 <未定义目录>", "<未定义目录>" in unk)
