...
# 测试隔离（统一开关 OPENCODE_TEST_HOME）：gate 子进程的所有运行时数据路径指向临时目录（run 时继承）
TEST_TMP = tempfile.mkdtemp(prefix="gate_test_")
os.environ["OPENCODE_TEST_HOME"] = TEST_TMP
LOG = os.path.join(TEST_TMP, "evolution_log.txt")
SNAPDIR = os.path.join(TEST_TMP, "gate_snap")
os.makedirs(SNAPDIR, exist_ok=True)
# 预置最小结构化流水（经验健康引擎扫描数据源；普通标题防与"测试经验"过滤混淆——该过滤仅针对真实流水历史污染）
open(LOG, "w", encoding="utf-8").write(
    "[2026-01-01] 网关鉴权链路预算经验\n- 状态：active\n- 核心关键词：网关鉴权、链路预算\n- 归属：evolution_skill\n\n"
    "[2026-01-02] 废弃机制处理经验\n- 状态：deprecated\n- 核心关键词：废弃机制\n- 归属：evolution_skill\n\n")
REAL_ELOG = os.path.join(CFG, "skills", "default", "evolution_skill", "evolution_log.txt")
pass_n, fail_n = 0, 0
def check(name, cond):
...
_bare = os.path.join(SNAPDIR, "gate_bare_declare.json")
...
# 3. 模拟改动：创建临时规则文件（用非测试前缀名 ztmp_gate_skill——_gate_test_skill* 前缀已被门禁忽略防噪音）
tmp_skill = os.path.join(CFG, "skills", "ztmp_gate_skill")
os.makedirs(tmp_skill, exist_ok=True)
tmp_md = os.path.join(tmp_skill, "SKILL.md")
open(tmp_md, "w", encoding="utf-8").write("---\nname: ztmp_gate_skill\ndescription: gate test\n---\n# test\n")
...
open(tmp_md, "w", encoding="utf-8").write("---\nname: ztmp_gate_skill\ndescription: gate test\n---\n# test v2\n")
r = run("--check", sid)
check("有改动时检测到", "检测到" in r.stdout and "ztmp_gate_skill" in r.stdout)
...
snaps_before = glob.glob(os.path.join(SNAPDIR, "gate_" + sid + ".json"))
check("模拟：快照残留（门禁未执行）", len(snaps_before) == 1)
r = run("--drain")
check("drain 检测到残留快照", "残留快照" in r.stdout and sid in r.stdout)
snaps_after = glob.glob(os.path.join(SNAPDIR, "gate_" + sid + ".json"))
check("drain 补跑后快照清理", len(snaps_after) == 0)
r = run("--drain")
snaps_sid = glob.glob(os.path.join(SNAPDIR, "gate_" + sid + ".json"))
...
snaps_left = glob.glob(os.path.join(SNAPDIR, "gate_" + sid + "-m*.json"))
...
# 9. 新增/删除文件检测（A+C 方案 2026-08-28：新增文件适配决策的前置检测；测试临时产物 _gate_* 前缀被忽略）
r = run("--snapshot", sid)
tmp_skill2 = os.path.join(CFG, "skills", "ztmp_new_skill_detect")
os.makedirs(tmp_skill2, exist_ok=True)
open(os.path.join(tmp_skill2, "SKILL.md"), "w", encoding="utf-8").write("---\nname: ztmp_new_skill_detect\ndescription: gate new test\n---\n# new\n")
new_tool = os.path.join(CFG, "tools", "ztmp_new_tool_detect.py")
...
check("仅新增无既有改动时快照已清理", len(glob.glob(os.path.join(SNAPDIR, "gate_" + sid + ".json"))) == 0)
# 删除检测
r = run("--snapshot", sid)
os.remove(new_tool)
r = run("--check", sid)
check("删除文件检出（【删除文件】段输出）", "【删除文件】" in r.stdout and "ztmp_new_tool_detect" in r.stdout)
check("仅删除无既有改动时快照已清理", len(glob.glob(os.path.join(SNAPDIR, "gate_" + sid + ".json"))) == 0)
# 测试临时产物忽略（防门禁噪音）
import shutil
r = run("--snapshot", sid)
tmp_fixture = os.path.join(CFG, "skills", "_gate_test_skill_fixture")
os.makedirs(tmp_fixture, exist_ok=True)
open(os.path.join(tmp_fixture, "SKILL.md"), "w", encoding="utf-8").write("---\nname: _gate_test_skill_fixture\ndescription: fixture\n---\n# f\n")
r = run("--check", sid)
check("_gate_test_skill* 测试产物不进门禁（忽略）", "_gate_test_skill_fixture" not in r.stdout)
shutil.rmtree(tmp_fixture, ignore_errors=True)

# 清理临时 skill（try/finally 保证：即使中途断言失败也清理，防残留污染 regedit 测试）
for _d in ("_gate_test_skill", "_gate_test_skill2", "ztmp_new_skill_detect", "ztmp_gate_skill"):
