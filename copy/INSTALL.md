...
| 7 路径改写 | `-NoPathRewrite` | 把 skill 文本中旧机路径（`C:\Users\<用户名>\...`、`E:\openCodeDefault\temp`）替换为新机实际路径 |
...
```
> 手动部署后，若新电脑用户名不是 `<用户名>`，需自行把 skill 与脚本里的旧路径全局替换为新路径（脚本方式 A 的"阶段 7"会自动完成这一步）。
